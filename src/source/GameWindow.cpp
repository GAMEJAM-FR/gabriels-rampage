...
bool GameWindow::checkProjectile(const Projectile &ejac)
{
  ...
  while (ct < this->_entities.size())
    {
      if (ejac.getX() >= this->_entities[ct]->getX() &&
	  ejac.getX() <= this->_entities[ct]->getX() + 2 * (int)this->_entities[ct]->getHitbox() &&
	  ejac.getY() >= this->_entities[ct]->getY() &&
	  ejac.getY() <= this->_entities[ct]->getY() + 2 * (int)this->_entities[ct]->getHitbox())
	{
